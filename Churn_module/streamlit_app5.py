...
tab1, tab2, tab3, tab4, tab5, tab6 = st.tabs([
    ...
    "6. Categorical Handling & Final DF ✨"
])
        ...
        if st.session_state.cols_to_remove:
            ...
            if st.button("Confirm and Remove Selected Columns"):
                ...
                if cols_to_actually_drop:
                    ...
                    st.info("The DataFrame has been updated. You can now proceed to 'Categorical Handling' or further steps.")
                else:
...
with tab6:
    st.header("6. Categorical Column Handling & Final DataFrame ✨")
    st.write("Choose how to handle your categorical columns: you can drop them or apply One-Hot Encoding.")
    ...
    if st.session_state.df_processed is not None:
        ...
        if not categorical_cols_available:
            ...
            st.markdown("---")
            st.subheader("Select Action for Each Categorical Column:")
            ...
            if st.button("Apply Categorical Transformations"):
                dropped_count = 0
                encoded_count = 0
                ...
                for col in categorical_cols_available:
                    ...
                    if action == "Drop":
                        if col in df_temp_processed.columns: # Check if column still exists
                            df_temp_processed = df_temp_processed.drop(columns=[col])
                            dropped_count += 1
                    elif action == "One-Hot Encode":
                        if col in df_temp_processed.columns: # Check if column still exists
                            # Ensure the column is of type 'object' or 'category' before encoding
                            # Sometimes, 'bool' might be caught as categorical, but get_dummies handles it fine
                ...
                st.success(f"Categorical transformations applied! Dropped {dropped_count} columns, One-Hot Encoded {encoded_count} columns. ✨")
                
                # Re-identify column types after transformations
                st.session_state.col_types = get_column_types(st.session_state.df_processed)
                
                st.subheader("DataFrame after Categorical Handling (first 5 rows):")
                st.dataframe(st.session_state.df_processed.head())
                ...
                st.info("Select actions for your categorical columns and click 'Apply Categorical Transformations'.")
