...
tab1, tab2, tab3, tab4, tab5, tab6 = st.tabs([
    ...
    "4. Collinearity & VIF",
    "5. Apply Changes",
    "6. Cat & Datetime" # Shorter tab names
])
        ...
        if not all_problematic_cols:
            ...
            if removal_option == "Automatically select columns for removal":
                ...
                if cols_auto_remove:
                    ...
                    st.info("These columns are marked for automatic removal. Proceed to 'Apply Changes' tab to confirm and remove.")
                else:
                ...
                if manual_selection_options:
                    ...
                    if selected_for_removal:
                        ...
                        for col in selected_for_removal:
                            st.code(col)
                        st.info("Proceed to 'Apply Changes' tab to confirm and remove.")
                    else:
        ...
        if st.session_state.cols_to_remove:
            ...
            if st.button("Confirm and Remove Selected Columns"):
                ...
                if cols_to_actually_drop:
                    ...
                    st.info("The DataFrame has been updated. You can now proceed to 'Cat & Datetime' or further steps.")
                else:
...
with tab6:
    st.header("6. Categorical & Datetime Handling & Final DataFrame ✨")
    st.write("Choose how to handle your categorical and datetime columns. Datetime columns are generally not used directly in models and can be dropped.")

    if st.session_state.df_processed is not None:
        categorical_cols_available = st.session_state.col_types["Categorical"]
        datetime_cols_available = st.session_state.col_types["Datetime"]
        ...
        if not categorical_cols_available:
            ...
            for col in categorical_cols_available:
                if col not in st.session_state.categorical_actions:
                    st.session_state.categorical_actions[col] = "Keep Original"
        ...
        if not datetime_cols_available:
            ...
            selected_datetime_cols_to_drop = st.multiselect(
                ...
                default=filtered_default_datetime_cols,
        ...
        if st.button("Apply Categorical & Datetime Transformations"):
            ...
            df_temp_processed = st.session_state.df_processed.copy()
...
st.sidebar.info(
    ...
    Cox Proportional Hazards model. Navigate through the tabs to perform each
    preprocessing step.
    """
)
st.sidebar.markdown("---")
