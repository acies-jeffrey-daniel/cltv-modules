...
if uploaded_file is not None:
    try:
        ...
        if st.session_state.df is None or uploaded_file.name != st.session_state.uploaded_file_name:
            ...
            # Clear previous collinearity state when a new file is uploaded
            if 'cols_to_remove' in st.session_state:
                del st.session_state.cols_to_remove
        else:
...
# --- Column Type Identification Logic ---
...
def check_collinearity_vif(df, numerical_cols, corr_threshold=0.8, vif_threshold=5.0):
    ...
    # Filter numerical_cols to only include those without NaN values,
    # as VIF cannot be calculated with NaNs.
    # For a real scenario, you'd handle NaNs (impute/remove) before VIF calculation.
    ...
    try:
        ...
        X = add_constant(df[clean_numerical_cols_for_vif])
        vif_data = pd.DataFrame()
        vif_data["feature"] = X.columns
        vif_data["VIF"] = [variance_inflation_factor(X.values, i) for i in range(X.shape[1])]

        # Exclude the constant column from VIF results
        ...
        st.error(f"Error calculating VIF: {e}. This often happens if all numerical columns are perfectly correlated or if there are constant columns. You might need to drop highly correlated columns first or ensure variance.")
        st.info("If you encounter this error, try removing highly correlated features manually first or check for columns with zero variance.")
...
if st.session_state.df is not None:
    ...
    col_types = get_column_types(st.session_state.df)
    ...
    st.markdown("---")
    st.header("3. Collinearity and VIF Analysis 📉")
    ...
    highly_correlated_pairs, high_vif_cols = check_collinearity_vif(st.session_state.df, numerical_cols, corr_threshold=0.8, vif_threshold=5.0)
    ...
    if not all_problematic_cols:
        ...
        if removal_option == "Automatically select columns for removal":
            ...
            # For automatic removal, a common strategy is to remove one from each highly correlated pair
            # or those with the highest VIF. For simplicity, we'll just list all detected ones.
            # In a real app, you'd implement a smarter heuristic.
            cols_auto_remove = sorted(list(all_problematic_cols))
            if cols_auto_remove:
                for col in cols_auto_remove:
                    st.code(col)
                # Store the columns to remove in session state for later application
                st.session_state.cols_to_remove = cols_auto_remove
                st.info("These columns are marked for automatic removal in subsequent steps.")
            else:
            ...
            if manual_selection_options:
                selected_for_removal = st.multiselect(
                    ...
                    default=[] # Pre-select nothing by default
                )
                ...
                st.session_state.cols_to_remove = [] # Ensure it's empty if no problematic cols
        else: # "Do nothing for now"
            st.info("No columns will be removed at this stage.")
            if 'cols_to_remove' in st.session_state:
                del st.session_state.cols_to_remove # Clear any previous selection
...
st.sidebar.info(
    ...
    data upload, column type identification, and now, collinearity and VIF analysis.
    """
)
st.sidebar.markdown("---")
